                    ...
                    if (input.addressDetails.Count > 0)
                    {
                        foreach (UpdateAddressDetail objAddress in input.addressDetails)
                        {
                            ...
                            //{
                                ...
                                else if (objAddress.addressID == "-1")
                                {
                                    sbAddress.Append("UPDATE member_address_detail SET isdeleted=1,deletion_date=now() where fk_member_profile_id=");
                                    sbAddress.Append(input.profileID);
                                    sbAddress.Append(" and Address_Type='");
                                    sbAddress.Append(objAddress.addressType);
                                    sbAddress.Append("';");
                                }
                                else
                                //If addressId != 0 then Update mode - Update record in DB with given addressID
                                {
        ...
        public static List<MemberListResult> GetDirectoryList(int masterUID, string grpID, string searchText, string updatedOn, int isSubGrpAdmin, string profileID)
        {
            try
            {
                ...
                if (isSubGrpAdmin == 0)
                {
                    ...
                    Result = _DBTouchbase.ExecuteStoreQuery<MemberListResult>("CALL V5_USPGetDirectoryList(?masterUID,?groupID,?SearchText,?updatedOn)", master_ID, groupID, SearchText, UpdatedOn).ToList();
                }
                else
                {
