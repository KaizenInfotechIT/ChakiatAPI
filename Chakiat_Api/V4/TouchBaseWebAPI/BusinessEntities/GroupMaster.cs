...
namespace TouchBaseWebAPI.BusinessEntities
{
    public class GroupMaster
    {
        ...
        public static List<MobilePopuplist> getMobilePupup(MobilePopuplist Obj)
        {
            try
            {
                MySqlParameter[] param = new MySqlParameter[1];
                param[0] = new MySqlParameter("?p_FkmasterID", Obj.FkmasterID);
                using (TouchBaseWebAPI.Data.row_productionEntities context = new TouchBaseWebAPI.Data.row_productionEntities())
                {
                    context.Connection.Open();
                    var Result = context.ExecuteStoreQuery<MobilePopuplist>("CALL API_getpopupDetailsForMobile(@p_FkmasterID)", param).ToList();
                    return Result;
                }
            }
            catch (Exception ex)
        ...
        public static int UpdateMobilePupupflag(MobilePopuplist Obj)
        {
            try
            {
                MySqlParameter[] parameters = new MySqlParameter[1];
                parameters[0] = new MySqlParameter("?p_FkmasterID", Obj.FkmasterID);

                string storeproc = "API_UpdateMobilePupupflag";
                int Result = MySqlHelper.ExecuteNonQuery(GlobalVar.strAppConn, CommandType.StoredProcedure, storeproc, parameters);
        ...
        public static masterlistResult GetMasterlist()
        {
            try
            {
                DataSet Result = MySqlHelper.ExecuteDataset(GlobalVar.strAppConn, CommandType.StoredProcedure, "USP_API_Mobile_Getmasterlist");

                DataTable dt_Branch = Result.Tables[0];
                DataTable dt_designation = Result.Tables[1];
                DataTable dt_entity = Result.Tables[2];
                DataTable dt_functional_role = Result.Tables[3];
                DataTable dt_department = Result.Tables[4];
                DataTable dt_country_master = Result.Tables[5];

                List<Branchlist> branchList = new List<Branchlist>();
                List<Designationlist> Designationlist = new List<Designationlist>();
                List<Entitylist> Entitylist = new List<Entitylist>();
                List<Functional_Role> Functional_Rolelist = new List<Functional_Role>();
                ...
                {
                    branchList = GlobalFuns.DataTableToList<Branchlist>(dt_Branch);
                }
                if (dt_Branch.Rows.Count > 0)
                {
                    Designationlist = GlobalFuns.DataTableToList<Designationlist>(dt_designation);
                }
                if (dt_Branch.Rows.Count > 0)
                {
                    Entitylist = GlobalFuns.DataTableToList<Entitylist>(dt_entity);
                }
                if (dt_Branch.Rows.Count > 0)
                {
                    Functional_Rolelist = GlobalFuns.DataTableToList<Functional_Role>(dt_functional_role);
                }
                if (dt_Branch.Rows.Count > 0)
                {
                    Departmentlist = GlobalFuns.DataTableToList<Department>(dt_department);
                }
                if (dt_Branch.Rows.Count > 0)
                {
                    Countrylist = GlobalFuns.DataTableToList<Country>(dt_country_master);
                }

                masterlistResult EbulletinList = new masterlistResult();
                EbulletinList.Branchlist = branchList;
                EbulletinList.Designationlist = Designationlist;
                EbulletinList.Entitylist = Entitylist;
                EbulletinList.Functional_Rolelist = Functional_Rolelist;
                EbulletinList.Departmentlist = Departmentlist;
                EbulletinList.Countrylist = Countrylist;
