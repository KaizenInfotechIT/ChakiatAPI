...
namespace TouchBaseWebAPI.Controllers
{
    public class GroupController : ApiController
    {
        ...
        {
            dynamic TBMasterListResult;
            List<object> ClubMonthlyReportListResult = new List<object>();

            try
            {
                masterlistResult Result = GroupMaster.GetMasterlist();

                if (ClubMonthlyReportListResult != null)
                {
                    TBMasterListResult = new { status = "0", message = "success", Result };
                }
                else
                {
                    TBMasterListResult = new { status = "0", message = "Record not found" };
                }
            }
            catch
            {
                TBMasterListResult = new { status = "1", message = "failed" };
            }

            return new { TBMasterListResult };
        }
    }
