...
namespace TouchBaseWebAPI.Controllers
{
    public class DistrictController : ApiController
    {
        ...
        [HttpPost]
        public object GetClassificationList_New(DistrictMemListInput obj)
        {
            dynamic ClassificationListResult;
            List<classificationOutput> Result = new List<classificationOutput>();
            int totalPages = 0;
            try
            {
                Result = District.GetClassificationList_New(obj, out totalPages);
                if (Result != null)
                {
                    ClassificationListResult = new { status = "0", message = "success", resultCount = Result.Count.ToString(), TotalPages = obj.pageNo.ToString(), currentPage = obj.pageNo.ToString(), Result };
                }
                else
                {
                    ClassificationListResult = new { status = "0", message = "Record not found" };
